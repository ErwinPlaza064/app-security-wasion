...
export default function ActiveVisitorsTable({ visitors, onExit, tabToggle, title = "Personal en Planta" }) {
    ...
    const activeCompanies = [
        ...new Set(
            visitors
                .map((v) => v.external_person?.company?.name || (['resignation', 'settlement', 'clearance', 'no_badge'].includes(v.type) ? 'INTERNO' : null))
                .filter(Boolean),
        ),
    ].sort();
    ...
    const filteredVisitors = visitors.filter((v) => {
        ...
        const companyName = v.external_person?.company?.name || (['resignation', 'settlement', 'clearance', 'no_badge'].includes(v.type) ? 'INTERNO' : '---');

        const matchesSearch =
            name?.toLowerCase().includes(tableSearch.toLowerCase()) ||
            companyName?.toLowerCase().includes(tableSearch.toLowerCase());
                ...
                <div className="flex flex-col items-end gap-3">
                    <div className="flex flex-col sm:flex-row gap-2">
                        ...
                        <div className="relative">
                            ...
                            />
                            <svg
                                className="w-3.5 h-3.5 text-gray-300 absolute left-4 top-1/2 -translate-y-1/2"
                                fill="none"
                                stroke="currentColor"
                                viewBox="0 0 24 24"
                                aria-hidden="true"
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-gray-50/50 uppercase">
                                    <th className="px-8 py-5 text-[9px] font-black text-gray-400 tracking-widest">
                                        Nombre
                                    </th>
                                    <th className="px-8 py-5 text-[9px] font-black text-gray-400 tracking-widest">
                                        Empresa
                                    ...
                                    <th className="px-8 py-5 text-[9px] font-black text-gray-400 tracking-widest text-center">
                                        Entrada
                                    </th>
                                    <th className="px-8 py-5 text-[9px] font-black text-gray-400 tracking-widest text-right">
                                        Salida
                                    </th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-50">
                                {filteredVisitors.map((v) => (
                                    <tr
                                        key={v.id}
                                        className="hover:bg-gray-50/30 transition-colors uppercase"
                                    >
                                        <td className="px-8 py-5 font-black text-[10px] text-gray-900">
                                            {v.external_person?.full_name ||
                                                v.visiting_person}
                                        </td>
                                        <td className="px-8 py-5 font-black text-[10px] text-primary">
                                                    ...
                                                    {v.type === "contractor"
                                                        ...
                                                        : v.type === "supplier"
                                                          ...
                                                          : v.type ===
                                                              "resignation"
                                                            ? "RENUNCIA"
                                                            : v.type === "settlement" ? "FINIQUITO" : v.type === "clearance" ? "PASE DE SALIDA"
                                                              : v.type === "no_badge"
                                                                ? "SIN GAFETE"
                                                                : "VISITANTE"}
                                                </span>
